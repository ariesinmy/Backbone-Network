	...
	//while(counter < redNodeCnt){
		for(int i = 0; i < num_vertex; i++){
			...
			for(unsigned int j = 0; j < num_vertex; j++){
				...
				//cout << srcID << " ";
				while(curID != dstID){
					if(CdsNodeList[curID].status != 1){ //若該node不是黑點 
...
int main()
{
  ...
  //creat a MisNodeList array
	...
	/*//印出所有的MisNodeList的status 
	for(int i=0; i<nodeCnt; i++){
		MisNodeList[i].printMisStatus(); 
	}*/
	...
	/*//印出所有的CdsNodeList的status
	printf("CDS starting status:\n");
	for(int i=0; i<nodeCnt; i++){
		CdsNodeList[i].printCdsStatus(); 
	} */
	...
	while(cnt < output){
			unsigned int srcProxy, dstProxy; //起點和終點的代理人 
